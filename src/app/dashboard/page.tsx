      ...
      <div className="grid min-h-screen" style={{ gridTemplateColumns: "248px 1fr" }}>
        ...
        <aside className="hidden md:flex flex-col sticky top-0 h-screen" style={{ background: C.card, borderRight: `1px solid ${C.line}`, padding: "18px 14px" }}>
          ...
          <nav className="mt-1 flex flex-col gap-0.5">
            <span className={navItem} style={{ background: C.brandSoft, color: C.brandDeep, fontWeight: 700 }}>
              <LayoutDashboard className="h-5 w-5" /> ダッシュボード
            </span>

            {/* 試験を選ぶ（常時展開ツリー・正式名称） */}
            <div className="px-3 pb-1 pt-3 text-[12px] font-bold tracking-wide" style={{ color: C.muted }}>試験を選ぶ</div>
            {basicExams.map((ex) => {
              ...
              return (
                <div key={ex.id}>
                  <button
                    ...
                    className="flex w-full items-center rounded-[10px] py-2.5 pl-5 pr-3 text-left text-[15px] transition-colors"
                    style={on ? { background: C.brandSoft, color: C.brandDeep, fontWeight: 700 } : { color: C.ink, fontWeight: 500 }}
                  >
                    {ex.name}
                  </button>
                  {/* A: 常時 午前/午後 サブをインデント表示 */}
                  {(EXAM_SUB[ex.id] ?? []).map((s) => (
                    <Link key={s.href} href={s.href} className="flex items-center gap-1.5 rounded-[10px] py-1.5 pl-11 pr-3 text-[13.5px]" style={{ color: C.muted }}>
            ...
            {/* G: AI弱点分析ナビは削除（各試験ダッシュボードで見られるため） */}
            ...
            {isGuest ? (
              ...
              <div className="mb-[18px] flex flex-col items-start gap-4 rounded-[14px] p-4 sm:flex-row sm:items-center sm:px-5" style={{ background: C.brandSoft, border: "1px solid #CFE0FB" }}>
                ...
                <div className="flex-1">
                  ...
                  {hasData && active.top ? (
                    <>
                      <div className="mt-0.5 text-[15px] font-bold">弱点の <span style={{ color: C.bad }}>「{active.top.category}」</span> を重点演習しましょう</div>
                ...
                <Link href={hasData && active.top ? studyHref(activeExam, active.top.category) : `/challenge/${activeExam}`} className="flex items-center gap-1.5 whitespace-nowrap rounded-[11px] px-5 py-3 text-[13.5px] font-bold text-white" style={{ background: C.brand }}>
                  {hasData && active.top ? `「${active.top.category}」を演習` : "まず解いてみる"}<ArrowRight className="h-4 w-4" />
                </Link>
              </div>
            )}
